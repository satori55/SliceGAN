...
# 定义2D TIFF切片的文件名列表
tiff_slices = eachFile("data")
sort_slices = sorted(tiff_slices, key=slice_rank)
print(sort_slices)

# 读取所有切片并存储在一个列表中
images = [tifffile.imread("data/" + slice_file) for slice_file in sort_slices]

image_crop = [element[107:363, 191:447, 1] for element in images]

# 将列表中的图像堆叠成一个3D数组
image_stack = np.stack(image_crop, axis=0)

# image_stack = einops.rearrange(image_stack, "z y x -> y z x")

image_np  = image_stack[50]
image_np_transformed = np.zeros_like(image_np)
