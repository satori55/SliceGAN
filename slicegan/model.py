...
def train(pth, imtype, datatype, real_data, Disc, Gen, nc, l, nz, sf):
    ...
    dataloaderx = torch.utils.data.DataLoader(
        dataset_xyz[0], batch_size=batch_size, shuffle=True, num_workers=workers
    )
    dataloadery = torch.utils.data.DataLoader(
        dataset_xyz[1], batch_size=batch_size, shuffle=True, num_workers=workers
    )
    dataloaderz = torch.utils.data.DataLoader(
        dataset_xyz[2], batch_size=batch_size, shuffle=True, num_workers=workers
    )
    ...
    for epoch in range(num_epochs):
        ...
        for i, (datax, datay, dataz) in enumerate(
            zip(dataloaderx, dataloadery, dataloaderz), 1
        ):
            ...
            for dim, (netD, optimizer, data, d1, d2, d3) in enumerate(
                ...
                fake_data_perm = fake_data.permute(0, d1, 1, d2, d3).reshape(
                    l * D_batch_size, nc, l, l
                )
                out_fake = netD(fake_data_perm).mean()
                gradient_penalty = util.calc_gradient_penalty(
                    netD,
                    real_data,
                    fake_data_perm[:batch_size],
                    batch_size,
                    l,
                    device,
                    Lambda,
            ...
            if i % int(critic_iters) == 0:
                ...
                for dim, (netD, d1, d2, d3) in enumerate(
                    ...
                    fake_data_perm = fake.permute(0, d1, 1, d2, d3).reshape(
                        l * batch_size, nc, l, l
                    )
